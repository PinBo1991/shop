<%@page language="java" pageEncoding="utf-8"%>
<%@include file="/WEB-INF/jsp/include.jsp"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<title>用户新增</title>
<%--author:GZZ,DATE:2014-02-16 00:23:30--%>
</head>
<body>
	<form:form name="form1" method="post" commandName="user">
		<div class="ti">用户新增</div>
		<table class="dataTab">
			<tr>
				<td class="right30">姓名</td>
				<td class="left30"><form:input path="name" maxlength="50" class="text-m"   /><font color="#CE0000">*</font></td>
				<td class="left30_red" id="name_td"><form:errors path="name" /></td>
			</tr>
			<tr>
				<td class="right30">登录名</td>
				<td class="left30"><form:input path="login_id" maxlength="50"  class="text-m"  /><font color="#CE0000">*</font></td>
				<td class="left30_red" id="login_id_td"><form:errors path="login_id" /></td>
			</tr>
			<tr>
				<td class="right30">性别</td>
				<td class="left30"><form:select path="sex_stat" items="${sexmap}" class="sel"/><font color="#CE0000">*</font></td>
				<td class="left30_red" id="sex_stat_td"><form:errors path="sex_stat" /></td>
			</tr>
			<tr>
				<td class="right30">部门1</td>
				<td class="left30"><input name="org_name1" id="org_name1"  readonly="readonly" onClick="getDeptRef('org_name1','dept_id1');"
					class="inputref" /><input name="dept_id1" id="dept_id1"/><font color="#CE0000">*</font></td>
				<td class="left30_red" id="dept_id_td"><form:errors path="dept_id" /></td>
			</tr>
						<tr>
				<td class="right30">部门2</td>
				<td class="left30"><input name="org_name2" id="org_name2"  readonly="readonly" onClick="getDeptRef('org_name2','dept_id2');"
					class="inputref" /><input name="dept_id2" id="dept_id2" /><font color="#CE0000">*</font></td>
				<td class="left30_red" id="dept_id_td"><form:errors path="dept_id" /></td>
			</tr>
			<tr>
				<td class="right30">联系电话</td>
				<td class="left30"><form:input path="tel" maxlength="50" class="text-m"   /></td>
				<td class="left30_red" id="tel_td"><form:errors path="tel" /></td>
			</tr>
			<tr>
				<td class="right30">备注</td>
				<td class="left30"><form:textarea path="remark" maxlength="200"  class="tarea" />
					<div id="remark_div">最大长度为200个字节</div></td>
				<td class="left30_red" id="remark_td"><form:errors path="remark" /></td>
			</tr>
		</table>
		<center class="btn_div">
			<input type="button" class="bnt" onclick="insert_onclick()" value="保存"/>
			<input type="button" class="bnt" onclick="list_back()"	value="返回"/>
		</center>
	</form:form>
</body>
<script type="text/javascript">
	$('#dept_id').bind('input propertychange', function() {notBlank('dept_id', '部门');});
	$('#login_id').bind('input propertychange', function() {notBlank('login_id', '登录名');});
	$('#name').bind('input propertychange', function() {notBlank('name', '姓名');});
	$('#remark').bind('input propertychange', function() {notifyLength(this);});
	
	
	function insert_onclick() {//保存记录
		form1.action = "${webPath}/user/insert";
		//请修改或替换如下验证方法
		notBlank("dept_id", "部门");//非空验证
		notBlank("login_id", "登录名");//非空验证
		notBlank("name", "姓名");//非空验证
		if (count > 0) {
			count = 0;
			return false;
		}
		form1.submit();
	}
	function list_back() {//返回列表
		form1.action = "${webPath}/user/list";
		form1.submit();
	}
	function getDeptRef(name,id) {
		var text, url, winWidth, winHeight, widId;
		text = "部门";
		url = "${webPath}/sysdept/ref?rname="+name+"&rvalue="+id;
		winWidth = 500;
		winHeight = $(top.window).height() - 20;
		widId = 'win001';
		top.ShowWin(text, url, "${webPath}", winWidth, winHeight, widId);
	}
</script>
</html>